...
void MMU::setDividerRegister(uint8_t data)
{
    memory.at(DIV_addr) = data;
}

uint8_t MMU::getDividerRegister()
{
    return memory.at(DIV_addr);
}
...
uint8_t MMU::getTimerCounterRegister()
{
    return memory.at(TIMA_addr);
}
