...
uint8_t PPU::getYPositionFromOAM(uint32_t entry)
{
    return (entry & 0xFF) - 16;
}
...
uint8_t PPU::getSpriteTileDataYOffset(uint8_t y_position, uint8_t sprite_flags)
{
    ...
    if (colour_id == 0) {
        return false;
               ...
               display_buffer_colour_id.at(
                   getFlattenedDisplayBufferColourIDIndex(pixel, scanline)) !=
                   0) {
        return false;
    }
    return true;
