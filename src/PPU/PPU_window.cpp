...
void PPU::renderWindow()
{
    ...
    // Note: horizontal window position represents the starting x position of
    // the rectangle window
    for (uint8_t pixel = getWindowHorizontalPosition();
         pixel < kLCDViewPortWidth; ++pixel) {
        uint16_t window_tile_number_x_offset =
            getWindowTileNumberXOffset(pixel);
        uint8_t tile_number =
