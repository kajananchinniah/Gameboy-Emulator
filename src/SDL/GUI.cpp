...
inline void checkSDL(int status, int line_number, const char *file_name)
{
    if (status < 0) {
        throw std::runtime_error(
            "SDL failed at: " + std::to_string(line_number) + " in file: " +
            std::string(file_name) + "! Error: " + std::string(SDL_GetError()));
    }
}
...
{
    if (!ptr) {
        throw std::runtime_error("Error: memory allocation failed at: " +
...
struct GUI::GUIImpl {
    ...
    GUIImpl()
        : window_{nullptr, SDL_DestroyWindow},
          rend_{nullptr, SDL_DestroyRenderer}, texture_{nullptr,
                                                        SDL_DestroyTexture}
    {
    }
    ...
    void init(int window_width, int window_height)
    {
        ...
        int render_index = -1;
        rend_.reset(
            SDL_CreateRenderer(window_.get(), render_index, render_flags));
...
JoyPadButton GUI::getKeyboardInput()
{
    return gui_impl_->getKeyboardInput();
}
bool GUI::shouldQuit()
{
    return gui_impl_->shouldQuit();
}
bool GUI::shouldSave()
{
    return gui_impl_->shouldSave();
}
